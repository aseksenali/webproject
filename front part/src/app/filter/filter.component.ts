import {Component, EventEmitter, OnInit, Output} from '@angular/core';
import {BookService} from '../services/book.service';
import {FilterService} from '../services/filter.service';
import {from} from 'rxjs';
import {AuthService} from '../services/auth.service';
import {Router} from '@angular/router';

@Component({
  selector: 'app-filter',
  templateUrl: './filter.component.html',
  styleUrls: ['./filter.component.css']
})
export class FilterComponent implements OnInit {
  genres: string[];
  selectedGenres = new Set<string>();
  genreCounter = 0;

  constructor(private bookService: BookService, private filterService: FilterService, private router: Router) {
  }

  ngOnInit(): void {
    this.getGenres();
    this.router.events.subscribe(() => this.getGenres());
  }

  getGenres() {
    this.bookService.getGenres().subscribe(genres => this.genres = genres.map<string>((genre) => genre.name.toUpperCase()).sort());
  }

  onGenreSelect(e): void {
    if (e.target.checked) {
      this.genreCounter += 1;
      this.selectedGenres.add(e.target.defaultValue.toUpperCase());
    } else {
      this.genreCounter -= 1;
      this.selectedGenres.delete(e.target.defaultValue.toUpperCase());
    }
    if (this.genreCounter !== 0) {
      this.filterService.onGenreFilter(Array.from(this.selectedGenres));
    } else {
      this.filterService.disableGenreFilter();
    }
  }
}
